...
kSocketStringTestCases = [
    ...
    ('r-g-x-x b-x', 'Weapons', 'R-G-X-X B-X', 'r-g-x-x_b-x__weapons',
            ['Class == {}'.format(parse_helper.ConvertValuesListToString(consts.kWeaponItemClasses)),
             'Sockets >= 6RGB', 'SocketGroup >= 4RG', 'SocketGroup >= 2B']),
]
...
def TestGenerateSocketConditions():
    for socket_string, item_slot, _, _, expected_condition_lines in kSocketStringTestCases:
        condition_lines = socket_helper.GenerateClassAndSocketConditions(socket_string, item_slot)
...
def TestGenerateSocketConditionsInvalidInput():
    for socket_string in kInvalidSocketStrings:
        try:
            conditions_list = socket_helper.GenerateClassAndSocketConditions(socket_string, item_slot='any')
        except RuntimeError:
            # This should happen
            pass
        else:
